...
<div class="row">        
        ...
        <div class="col-md-8 center login-header">
            <h2 align="center">Classification of Tweets</h2>
        </div>
        <!--/span-->
    ...
    <div class="row">
        <div class="well col-md-9  login-box">
    		...
    		<tbody>
    		<%
    			ArrayList<Tweet> twitList = (ArrayList<Tweet>)session.getAttribute("twitList");
    			if(twitList!=null&&twitList.size()>0){
    				for(int i=0;i<twitList.size();i++){
    							
    		%>
    		<tr>
    			<td><%=twitList.get(i).getTweetId() %></td>
        		<td><%=twitList.get(i).getMessage() %></td>
       			<td><%=twitList.get(i).getStemming() %></td>
        		<td><%=twitList.get(i).getResult() %></td>
       			<td><%=twitList.get(i).getBayesResult() %></td>
